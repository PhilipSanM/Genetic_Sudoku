...
class Gui(Tk):
    ...
    def solve_using_GA(self):
        ...
        # TODO CAMARA EMI >;( HUEVOS ATODOS

        start = time.time()
        ga_solver = GeneticAlgorithm()
        board = self.get_sudoku_matrix(self.init_solution.copy())

        #ga_solution, it = ga_solver.sudoku_ga(board, 150, 1000,  0.2, 0.1, 0.3, 0.05)
        self.ga_solution  = ga_solver.sudoku_ga(board, population, 1000,  0.2, 0.1, 0.3, 0.05)
